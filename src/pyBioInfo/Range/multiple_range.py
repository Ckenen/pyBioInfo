...
class MRange(GRange):
    def __init__(self, chrom=None, start=None, end=None, name=None, strand=None, 
        ...
        if blocks_array is None:
            ...
            for blocks1 in blocks_array:
                BlockTools.check_valid_blocks(blocks1)
            ...
            blocks = tuple([(bstart, bend) for bstart, bend in BlockTools.suture(array2, check=False)])
        assert blocks
    ...
    def coincide_from(self, other):
        for blocks in self._blocks_array:
            if not BlockTools.coincide(other.blocks, blocks):
                return False
        return True
